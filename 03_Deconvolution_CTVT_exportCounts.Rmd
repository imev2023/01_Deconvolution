...
length(unique(goi))==length(unique(rowDataObjMapped$Symbol))
length(unique(goi))==length(unique(rowDataObjMapped$ID))
length(unique(goi))==length(rownames(rowDataObjMapped))

enscaf <- rowDataObjMapped$ID
```
...
write.table(
  snCountsFinalNoBad, 
  file = paste0(resDir, 
                "/0_scadenInput/snCountsNoBad/snCountsNoBad_counts.txt"), 
  sep = "\t", 
  quote = FALSE, 
...
write.table(
  snAnnoFinalNoBad, 
  file = paste0(resDir,
                "/0_scadenInput/snCountsNoBad/snCountsNoBad_celltypes.txt"), 
  sep = "\t", 
  quote = FALSE,
  col.names = T,  # Ensures first column (gene names) gets no name, 
  row.names = F
...
write.table(
  snSamplesFinalNoBad, 
  file = paste0(resDir,
                "/0_scadenInput/snCountsNoBad/snCountsNoBad_samples.txt"), 
  sep = "\t", 
  quote = FALSE,
  col.names = T,  # Ensures first column (gene names) gets no name, 
  row.names = F
