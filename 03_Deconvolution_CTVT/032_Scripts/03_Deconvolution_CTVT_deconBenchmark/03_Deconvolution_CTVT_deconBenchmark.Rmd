...
setdiff(rownames(snCountsFinal), rownames(bulk))
```
...
musicRes <- MuSiC$MuSiC$P %>%
  ...
  mutate("deconvBenchResType" = "P", # P for proportion
         "Method" = "MuSiC", 
         "Parameters" = "Default", 
         "Reference" = "subSampledHVGs",
         "Seed"=123)

# Rbind proportion outputs
musicResGround <- rbind(musicRes, snCellPercentages_subSample_relProp)
...
# rnasieveResGround <- rbind(rnasieveRes, snCellPercentages_subSample_relProp)
# 
...
# Rbind proportion outputs to compute error metrics.
adroitResGround <- rbind(adroitResP, snCellPercentages_subSample_relProp)
...
```{r}
# digitalDLSorter <- runDeconvolution("digitalDLSorter", 
#                          bulk = bulkCountsFinal,  # 20 Samples
#                          singleCellExpr = snCountsFinal, 
#                          singleCellLabels = snAnno,
#                          dockerArgs = c("--cpus=8.0",
...
#                                         "--memory-reservation=8G"))
# 
# ddlsRes <- digitalDLSorter  %>%
#   mutate()
```

#### 3.1.1.2 Set 2
Of note, requires defining a signature matrix.
Interesting to see how many of the signature genes will overlap with the HVGs list using the full sn matrix. 
...
# 
# # Rbind proportion outputs to compute error metrics.
# dwlsResGround <- rbind(dwlsResP, snCellPercentages_subSample_relProp)
# 
# # snCellPercentages have already been rbound to masterPropDf in the "MuSiC" chunk above, so will filter these out after error calculations.
...
MIXTURE_fullRef <- runDeconvolution("MIXTURE", 
                         bulk = bulkCountsFinal,  # 20 Samples
                         signature = reference_HVGs$signature, # 195 genes creating reference from hvgs and all cells rather than subsample. This could be indicative that we need to loop over multiple seeds for the true best method.  
                         dockerArgs = c("--cpus=8.0",
                                        "-m=16G", 
                                        "--memory-reservation=8G"))
...
# # Rbind proportion outputs to compute error metrics.
# mixtureResGround <- rbind(mixtureResP_fullRef, snCellPercentages_subSample_relProp)
...
# 
# # Rbind proportion outputs to compute error metrics.
# # fardeepResGround <- rbind(fardeepResP_fullRef , snCellPercentages_subSample_relProp)
# 
# # snCellPercentages have already been rbound to masterPropDf in the "MuSiC" chunk above, so will filter these out after error calculations.
...
# 
# # Rbind proportion outputs to compute error metrics.
# lindeconseqResGround <- rbind(lindeconseqResP_fullRef, snCellPercentages_subSample_relProp)
# 
# # snCellPercentages have already been rbound to masterPropDf in the "MuSiC" chunk above, so will filter these out after error calculations.
...
# Rbind proportion outputs to compute error metrics.
bayesprismResGround <- rbind(bayesprismResP, snCellPercentages_subSample_relProp)
...
# Rbind proportion outputs to compute error metrics.
autogenesResGround <- rbind(autogenesResP, snCellPercentages_subSample_relProp)
...
snCellSubSampleRelProp <- rowSums(snCellCounts_subSample_relProp)
...
For each reference (ex. snCounts), we want to compute the average prop of cell types across all samples. 
```{r}
snCell <- rowSums(snCellCounts_subSample_relProp)
