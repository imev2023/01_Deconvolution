...
!Fix file paths!
```{r}
bulk <- read_table(paste0("~/gitClones/01_Deconvolution/03_Deconvolution_CTVT/033_Results/03_Deconvolution_CTVT_customScaden/0_scadenInput/03_Deconvolution_CTVT_customScaden_bulkCountsGoi.txt"), 
                   rown)
...
sampleTableNew <- data.frame(sampleName = sampleTableI$sampleName,
                             fileName = sampleTableI$fileName, 
                             type = sampleTableI$type) %>% 
  filter(!sampleName == "3145T1c")


#create dds object (gather htseq count output)
ddsHT_SC <- DESeqDataSetFromHTSeqCount(sampleTable = sampleTableNew,
                                       directory = "~/Desktop/1_Deconvolution/For_Nina/01_RNAseq_analysis/02_CTVT/Data_input_htseq_reverse/",
                                       design = ~ type)
...
rld <- rlog(ddsHT_SC, blind = TRUE)


normCounts <- counts(ddsHT_SC, normalized = TRUE)[goi,]
logCounts_rlog <- assay(rld)[goi,]
...
pbBySample <- scuttle::aggregateAcrossCells(objNoBad, objNoBad$)

pbBySampleAssay <- assay(pbBySample, "logcounts")

# pbBySampleCluster <- pseudobulk(objNoBad, group_by = c("Sample", manual.log.anot.fine))
```


## 3.2 Does expression in sn psuedobulk correlate with bulk expression?
```{r}

```

## 3.3 Does expression in pseudobulk per cell type and sample correlate with number of cells?
```{r}
