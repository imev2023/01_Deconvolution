...
test <- read.table(paste0(resDir, "/old/", fprefix, "_snCountsFinal_snAnno.txt"), 
                          sep = "\t")

test2 <- read.table(paste0(resDir, fprefix, "_snCountsFinalUnique_snAnno.txt"), 
                          sep = "\t")
...
cibersort$Cluster <- factor(music$Cluster, levels = rev(c("CTVT", 
                                                  "Myeloid", 
                                                  "Bcells",
                                                  "CAF.Endo",
                                                  "Tcells",
...
# Step 3: Plot
cibersortPlot <- ggplot(cibersort, aes(x = Sample, y = Proportion, fill = Cluster)) +
...
# Apply correction
matched <- cibersort %>%
  filter(Sample %in% unique(truePuritiesPlot$Sample)) %>%
  inner_join(truePuritiesPlot %>% rename("Proportion"="True" ),
             by = c("Sample", "Cluster")) %>%
  # Add prefix and shape
  mutate(Prefix = str_extract(Sample, "^[^T]+"),
         Shape = if_else(str_detect(Sample, "_n$"), "Nuc", "Cell"))


# Scatter plot
matchedPlot <- ggplot(matched, aes(x = True, y = Proportion)) +
  geom_point(aes(color = Prefix, shape = Shape), size = 2, alpha = 0.8) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray") +
  facet_wrap(~Cluster, nrow = 1) +
  coord_fixed(xlim = c(0, 1), ylim = c(0, 1)) +
  scale_shape_manual(values = c(Cell = 16, Nuc = 17)) + # 16 = circle, 17 = triangle
...
pdf(paste0(plotDir, "/scatter/", fprefix, "_cibersortRes_minExpr0_minGenes1_matched.pdf"),
    height=4, width=7)
print(matchedPlot)
dev.off()
```

#### 5.1.1.2 Bulk expression
```{r}
...
pdf(paste0(plotDir, "/scatter/", fprefix, "_cibersortResUnique_minExpr0_minGenes1_matched.pdf"),
    height=4, width=7)
print(matchedPlot)
dev.off()
```
...
pdf(paste0(plotDir, "/scatter/", fprefix, "_cibersortRes_minExpr10_minGenes2to10_matched.pdf"),
    height=4, width=7)
print(matchedPlot)
dev.off()
```
...
pdf(paste0(plotDir, "/scatter/", fprefix, "_cibersortRes_minExpr10_minGenes4to6_matched.pdf"),
    height=4, width=7)
print(matchedPlot)
dev.off()
```
...
pdf(paste0(plotDir, "/scatter/", fprefix, "_cibersortRes_minExpr50_minGenes2to10_matched.pdf"),
    height=4, width=7)
print(matchedPlot)
dev.off()
```
