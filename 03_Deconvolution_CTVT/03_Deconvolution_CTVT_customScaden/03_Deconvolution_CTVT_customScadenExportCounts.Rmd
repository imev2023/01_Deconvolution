...
#### 2.1.2.1 Mapped
```{r}
snCountsFinalNoBad <- assay(objNoBad, "counts") %>%
  as.data.frame() %>%
  rownames_to_column("rownames") %>%
  left_join(rowDataObjMapped) %>% 
  drop_na("ID") %>% 
...
write.table(
  snCountsFinalNoBad, 
  file = paste0(resDir, 
                "/0_scadenInput/snCountsNoBadq1q4/snCountsNoBadq1q4_counts.txt"), 
  sep = "\t", 
  quote = FALSE, 
...
write.table(
  snAnnoFinalNoBad, 
  file = paste0(resDir,
                "/0_scadenInput/snCountsNoBadq1q4/snCountsNoBadq1q4_celltypes.txt"), 
  sep = "\t", 
  quote = FALSE,
  col.names = T,  # Ensures first column (gene names) gets no name, 
  row.names = F
...
write.table(
  snSamplesFinalNoBad, 
  file = paste0(resDir,
                "/0_scadenInput/snCountsNoBadq1q4/snCountsNoBadq1q4_samples.txt"), 
  sep = "\t", 
  quote = FALSE,
  col.names = T,  # Ensures first column (gene names) gets no name, 
  row.names = F
...
write.table(
  snCountsFinalNoBad, 
  file = paste0(resDir, 
                "/0_scadenInput/snCountsNoBadq1q4Unique/snCountsNoBadq1q4Unique_counts.txt"), 
  sep = "\t", 
  quote = FALSE, 
  row.names = c(seq(from=0, to=nrow(snCountsFinalNoBad)-1, by=1)),
  col.names = NA  # Ensures first column (cell id's names) gets no name
...
write.table(
  snAnnoFinalNoBad, 
  file = paste0(resDir,
                "/0_scadenInput/snCountsNoBadq1q4Unique/snCountsNoBadq1q4Unique_celltypes.txt"), 
  sep = "\t", 
  quote = FALSE,
  col.names = T,  # Ensures first column (gene names) gets no name, 
  row.names = F
...
write.table(
  snSamplesFinalNoBad, 
  file = paste0(resDir,
                "/0_scadenInput/snCountsNoBadq1q4Unique/snCountsNoBadq1q4Unique_samples.txt"), 
  sep = "\t", 
  quote = FALSE,
  col.names = T,  # Ensures first column (gene names) gets no name, 
  row.names = F
...
bulkCountsGoiPurityNoBad <- bulkCountsFinal[enscaf,] %>%
  t() %>%
  as.data.frame() %>%
  rownames_to_column("ID") %>%
  mutate("Sample"=str_replace(ID, "_[np2]$", "")) %>%
...
write.table(
  bulkCountsGoiPurityNoBad, 
  file = paste0(resDir, "/0_scadenInput/", fprefix, "_bulkCountsGoiPurityNoBad.txt"), 
  sep = "\t", 
  quote = FALSE, 
  col.names = NA  # Ensures first column (gene names) gets no name
)
...
bulkCountsGoiPurityAndy <- bulkCountsFinal[enscaf,] %>%
  t() %>%
  as.data.frame() %>%
  rownames_to_column("ID") %>%
  mutate("Sample"=str_replace(ID, "_[np2]$", "")) %>%
...
bulkCountsGoiPurityNoBadAndy <- bulkCountsFinal[enscaf,] %>%
  t() %>%
  as.data.frame() %>%
  rownames_to_column("ID") %>%
  mutate("Sample"=str_replace(ID, "_[np2]$", "")) %>%
